...
st.set_page_config(
    page_title="Music Streaming Analytics Dashboard",
    page_icon="🎵",
    layout="wide",
...
def load_data():
    try:
        ...
        return {
            ...
            'momentum_score': momentum_score_df,
            'cohort_engagement': cohort_engagement_df,
            'geo_breakdown': geo_breakdown_df,
            'discovery_channel': discovery_channel_df
        }
...
def main():
    # Add header
    st.markdown("<h1 class='main-header'>Music Streaming Analytics Dashboard</h1>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        view_option = st.selectbox(
            ...
            ["Overview", "Artist Performance", "Geographic Analysis", "Audience Engagement", "Discovery Channels"]
        )
        
        if view_option in ["Artist Performance", "Geographic Analysis"]:
            if 'top_artists' in data:
                artists = sorted(data['top_artists']['artist'].unique())
                selected_artist = st.selectbox("Select Artist", artists)
        
        st.markdown("---")
        st.markdown("### About")
        st.markdown("This dashboard visualizes music streaming analytics data from April 1-17, 2025.")
    
    # Content based on selected view
    if view_option == "Overview":
        display_overview(data)
    elif view_option == "Artist Performance":
        display_artist_performance(data, selected_artist)
    elif view_option == "Geographic Analysis":
        display_geographic_analysis(data, selected_artist)
    elif view_option == "Audience Engagement":
        display_audience_engagement(data)
    elif view_option == "Discovery Channels":
        display_discovery_channels(data)
...
def display_overview(data):
    ...
    with col1:
        total_plays = data['top_artists']['total_plays'].sum()
        st.markdown(f"""
    ...
    with col2:
        total_engagements = data['top_artists']['total_engagements'].sum()
        st.markdown(f"""
        <div class='metric-card'>
    ...
    with col3:
        unique_users = data['top_artists']['unique_users'].sum()
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Unique Users</h3>
            <h2>{unique_users:,}</h2>
    ...
    with col4:
        engagement_rate = (total_engagements / total_plays) * 100
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Engagement Rate</h3>
            <h2>{engagement_rate:.2f}%</h2>
        </div>
        """, unsafe_allow_html=True)
    
    # Top Artists Section
    st.markdown("<h3 class='section-header'>Top 10 Most Engaged Artists</h3>", unsafe_allow_html=True)
    
    col1, col2 = st.columns([3, 2])
    
    with col1:
        top_10_artists = data['top_artists'].head(10)
        fig = px.bar(
            top_10_artists,
            x='artist',
            ...
            barmode='group',
            title='',
            labels={'value': 'Count', 'variable': 'Metric', 'artist': 'Artist'},
    ...
    with col2:
        ...
        st.dataframe(
            top_10_artists[['artist', 'total_plays', 'total_engagements', 'unique_users']],
    ...
    st.markdown("<h3 class='section-header'>Top Artists by Momentum Score</h3>", unsafe_allow_html=True)
    
    # Sort momentum score data
    momentum_data = data['momentum_score'].sort_values(by='momentum_score', ascending=False)
    ...
    with col1:
        fig = px.bar(
            ...
            y='momentum_score',
            title='',
            labels={'momentum_score': 'Momentum Score', 'artist': 'Artist'},
    ...
    with col2:
        # Bubble chart for growth metrics
        fig = px.scatter(
            momentum_data,
            x='play_growth_pct',
...
def display_artist_performance(data, selected_artist):
    st.markdown(f"<h2 class='section-header'>Artist Performance: {selected_artist}</h2>", unsafe_allow_html=True)
    
    # Filter data for selected artist
    artist_momentum = data['momentum_score'][data['momentum_score']['artist'] == selected_artist]
    artist_cohort = data['cohort_engagement'][data['cohort_engagement']['artist'] == selected_artist]
    artist_geo = data['geo_breakdown'][data['geo_breakdown']['artist'] == selected_artist]
    
    # Check if artist exists in the datasets
    if artist_momentum.empty and artist_cohort.empty and artist_geo.empty:
        st.warning(f"No detailed data available for {selected_artist}")
        return
    ...
    if not artist_momentum.empty:
        with col1:
            ...
            <div class='metric-card'>
                <h3>Plays</h3>
                <h2>{artist_momentum['plays'].values[0]:,}</h2>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            <div class='metric-card'>
                <h3>Play Growth</h3>
                <h2>{artist_momentum['play_growth_pct'].values[0]:.2f}%</h2>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col3:
            ...
            <div class='metric-card'>
                ...
                <h2>{artist_momentum['unique_listeners'].values[0]:,}</h2>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            ...
            <div class='metric-card'>
                ...
                <h2>{artist_momentum['momentum_score'].values[0]:.2f}</h2>
            </div>
            """, unsafe_allow_html=True)
    
    # Songs Performance
    if not artist_cohort.empty:
        st.markdown("<h3 class='section-header'>Songs Performance</h3>", unsafe_allow_html=True)
        ...
        with col1:
            fig = px.bar(
                artist_cohort,
                x='title',
                y=['total_plays', 'total_engagements', 'unique_users'],
                barmode='group',
                title='',
                labels={
                    'value': 'Count', 
                ...
                color_discrete_sequence=['#1DB954', '#191414', '#4285F4']
            )
            fig.update_layout(legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col2:
            fig = px.scatter(
                ...
                hover_name='title',
                size_max=60,
                title='Engagement Analysis by Song',
                labels={
                    'total_plays': 'Total Plays',
                    'engagement_per_user': 'Engagement per User',
                    'play_cohort': 'Play Cohort',
    ...
    if not artist_geo.empty:
        st.markdown("<h3 class='section-header'>Geographic Performance</h3>", unsafe_allow_html=True)
        
        # Group by country
        country_data = artist_geo.groupby('geo_country').agg({
            'total_plays': 'sum',
            'total_engagements': 'sum',
            'unique_users': 'sum'
        }).reset_index()
        
        country_data = country_data.sort_values(by='total_plays', ascending=False)
        ...
        with col2:
            fig = px.choropleth(
                ...
                color_continuous_scale="Viridis",
                title='',
                labels={'total_plays': 'Total Plays'}
            )
            fig.update_layout(geo=dict(showframe=False, showcoastlines=False))
            st.plotly_chart(fig, use_container_width=True)

def display_geographic_analysis(data, selected_artist):
    ...
    if artist_geo.empty:
        st.warning(f"No geographic data available for {selected_artist}")
        return
    
    # Country breakdown
    st.markdown("<h3 class='section-header'>Country Breakdown</h3>", unsafe_allow_html=True)
    ...
    with col1:
        fig = px.bar(
            ...
            x='geo_country',
            y='total_plays',
            title='Top 10 Countries by Plays',
            labels={'total_plays': 'Total Plays', 'geo_country': 'Country'},
            color='total_plays',
            color_continuous_scale='Viridis'
        )
        fig.update_layout(coloraxis_showscale=False)
    ...
    # Map visualization
    st.markdown("<h3 class='section-header'>Global Reach Map</h3>", unsafe_allow_html=True)
    ...
    # Region analysis for top countries
    if not artist_geo.empty:
        top_country = country_data['geo_country'].iloc[0]
        st.markdown(f"<h3 class='section-header'>Regional Analysis - {top_country}</h3>", unsafe_allow_html=True)
        
        # Filter for top country
        country_regions = artist_geo[artist_geo['geo_country'] == top_country]
        
        # Group by region
        region_data = country_regions.groupby('geo_region').agg({
            'total_plays': 'sum',
            'total_engagements': 'sum',
            'unique_users': 'sum'
        }).reset_index()
        
        region_data = region_data.sort_values(by='total_plays', ascending=False)
        ...
        with col1:
            fig = px.bar(
                ...
                y=['total_plays', 'total_engagements'],
                barmode='group',
                title=f'Top Regions in {top_country}',
                labels={
                    'value': 'Count',
                    'variable': 'Metric',
                    'geo_region': 'Region'
        ...
        with col2:
            fig = px.scatter(
                ...
                title=f'Engagement vs. Plays by Region in {top_country}',
                labels={
                    'total_plays': 'Total Plays',
                    'total_engagements': 'Total Engagements',
                    'unique_users': 'Unique Users'
...
def display_audience_engagement(data):
    st.markdown("<h2 class='section-header'>Audience Engagement Analysis</h2>", unsafe_allow_html=True)
    
    # Get cohort data
    cohort_data = data['cohort_engagement']
    
    # Engagement metrics
    col1, col2, col3 = st.columns(3)
    
    with col1:
        avg_engagement = cohort_data['engagement_per_user'].mean()
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Avg. Engagement per User</h3>
            <h2>{avg_engagement:.4f}</h2>
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        median_engagement = cohort_data['engagement_per_user'].median()
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Median Engagement per User</h3>
            <h2>{median_engagement:.4f}</h2>
        </div>
        """, unsafe_allow_html=True)
    
    with col3:
        max_engagement = cohort_data['engagement_per_user'].max()
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Max Engagement per User</h3>
            <h2>{max_engagement:.4f}</h2>
        </div>
        """, unsafe_allow_html=True)
    
    # Engagement distribution by play cohort
    st.markdown("<h3 class='section-header'>Engagement Distribution by Play Cohort</h3>", unsafe_allow_html=True)
    
    col1, col2 = st.columns(2)
    
    with col1:
        # Box plot of engagement per user by play cohort
        fig = px.box(
            cohort_data,
            x='play_cohort',
            y='engagement_per_user',
            color='play_cohort',
            title='Engagement per User Distribution by Play Cohort',
            labels={
                'play_cohort': 'Play Cohort',
                'engagement_per_user': 'Engagement per User'
            }
    ...
    with col2:
        ...
        fig = px.violin(
            ...
            box=True,
            points="all",
            title='Detailed Engagement Distribution',
            labels={
                'play_cohort': 'Play Cohort',
                'engagement_per_user': 'Engagement per User'
            }
    ...
    # Top engaged songs
    st.markdown("<h3 class='section-header'>Top 10 Songs by Engagement per User</h3>", unsafe_allow_html=True)
    
    top_engaged_songs = cohort_data.sort_values(by='engagement_per_user', ascending=False).head(10)
    ...
    with col2:
        st.markdown("<div class='card'>", unsafe_allow_html=True)
        st.dataframe(
            top_engaged_songs[['artist', 'title', 'engagement_per_user', 'play_cohort']],
            use_container_width=True,
            hide_index=True
        )
        st.markdown("</div>", unsafe_allow_html=True)
    
    # Scatter plot of engagement metrics
    st.markdown("<h3 class='section-header'>Engagement Analysis</h3>", unsafe_allow_html=True)
    ...
    fig = px.scatter(
        ...
        labels={
            'total_plays': 'Total Plays (log scale)',
            'engagement_per_user': 'Engagement per User',
            'play_cohort': 'Play Cohort',
            'unique_users': 'Unique Users'
        }
...
def display_discovery_channels(data):
    st.markdown("<h2 class='section-header'>Discovery Channels Analysis</h2>", unsafe_allow_html=True)
    
    # Get discovery channel data
    discovery_data = data['discovery_channel']
    
    # Aggregate by source tab
    tab_data = discovery_data.groupby('source_tab').agg({
        'event_count': 'sum'
    }).reset_index()
    
    tab_data = tab_data.sort_values(by='event_count', ascending=False)
    
    # Aggregate by section within tab
    section_data = discovery_data.sort_values(by='event_count', ascending=False)
    
    # Display tabs performance
    col1, col2 = st.columns(2)
    ...
    with col2:
        st.markdown("<h3 class='section-header'>Tab Engagement Counts</h3>", unsafe_allow_html=True)
    ...
    # Display sections performance
    st.markdown("<h3 class='section-header'>Top 15 Sections Across All Tabs</h3>", unsafe_allow_html=True)
    
    top_sections = section_data.head(15)
    ...
    # Select top 3 tabs
    top_tabs = tab_data['source_tab'].head(3).tolist()
    
    # Create multiselect with default as top 3
    selected_tabs = st.multiselect(
        "Select Source Tabs to Compare",
        tab_data['source_tab'].tolist(),
        default=top_tabs
    )
    
    if selected_tabs:
        # Filter for selected tabs
        filtered_sections = section_data[section_data['source_tab'].isin(selected_tabs)]
        
        # Get top 5 sections from each selected tab
        top_sections_by_tab = []
        for tab in selected_tabs:
            tab_sections = filtered_sections[filtered_sections['source_tab'] == tab].head(5)
            top_sections_by_tab.append(tab_sections)
        
        tab_sections_df = pd.concat(top_sections_by_tab)
        ...
        fig = px.bar(
            ...
            color='source_tab',
            barmode='group',
            title='Top 5 Sections by Selected Tabs',
            labels={
                'section': 'Section',
                'event_count': 'Event Count',
                'source_tab': 'Source Tab'
    ...
    # Additional insights
    st.markdown("<h3 class='section-header'>Discovery Channel Insights</h3>", unsafe_allow_html=True)
    
    col1, col2 = st.columns(2)
    
    with col1:
        # Count unique tabs and sections
        unique_tabs = len(discovery_data['source_tab'].unique())
        unique_sections = len(discovery_data['section'].unique())
        
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Discovery Channels</h3>
            <h4>{unique_tabs} Unique Tabs</h4>
            <h4>{unique_sections} Unique Sections</h4>
        </div>
        """, unsafe_allow_html=True)
    
    with col2:
        # Calculate effectiveness ratio (events per channel)
        events_per_channel = discovery_data['event_count'].sum() / unique_sections
        
        st.markdown(f"""
        <div class='metric-card'>
            <h3>Channel Effectiveness</h3>
            <h4>Average {events_per_channel:.2f} Events per Section</h4>
        </div>
        """, unsafe_allow_html=True)
