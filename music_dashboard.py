...
# --- Page Configuration ---
st.set_page_config(
    page_title="Artist Momentum Geo-Testing: Next Steps",
    layout="wide"
)

# --- Header ---
st.title("Artist Momentum: Proposed Next Wave of Tests")
st.markdown("""
This application outlines the proposed second wave of geo-specific tests. Based on initial findings, these queries are designed to be more sophisticated by incorporating **conversion ratios, high-intent proxies, and engagement velocity.**
""")
...
# --- Main Tabs for the new tests ---
tab1, tab2, tab3 = st.tabs(["West Africa: High-Conversion Hits", "U.S.: High-Intent Proxy", "Caribbean: Engagement Velocity"])

# --- West Africa Tab ---
with tab1:
    st.header("West Africa: Identifying High-Conversion Hits")
    st.markdown("""
    **Hypothesis:** The most promising emerging tracks are not just getting plays, but are exceptionally efficient at converting listeners into dedicated fans. We can measure this by analyzing the **download-to-play ratio**, while still ensuring the track has significant social proof through **reposts**.
    """)

    st.subheader("1. The Proposed Test")
    st.markdown("""
    * **Metrics:** `total_plays`, `playlist_adds`, `reposts`, `downloads`.
    * **Core Calculation:** A new metric, `downloads_per_1k_plays`, will be calculated to measure fan conversion efficiency.
    * **Thresholds:** We will maintain baseline thresholds for adds and downloads, but add a new requirement for `reposts` and filter for songs with enough plays to have a meaningful ratio.
    """)

    st.subheader("2. Polished Query to Test")
    st.code("""
    -- West Africa Test 2: High-Conversion Hits
    -- Focus: Adds Reposts, and a high Download-to-Play ratio to find efficient, viral tracks.
    WITH PlayCounts AS (
        -- First, get the total play counts for songs in the region
        SELECT
            object_id,
        ...
        WHERE event_date BETWEEN date_format(current_date - interval '30' day, '%Y%m%d')
          ...
          AND type = 'play'
          AND geo_country IN ('NG', 'GH')
        GROUP BY 1
    ),
    EngagementCounts AS (
        -- Next, get the engagement counts we care about
        SELECT
            m.music_id_raw,
    ...
    -- Finally, join them and calculate the ratio
    SELECT
        ec.artist_name,
        ec.song_name,
        pc.total_plays,
        ...
        ec.reposts,
        ec.downloads,
        -- Calculate downloads per 1000 plays to get a clean conversion rate
        (CAST(ec.downloads AS DOUBLE) / pc.total_plays) * 1000 AS downloads_per_1k_plays
    FROM EngagementCounts ec
    JOIN PlayCounts pc ON ec.music_id_raw = pc.object_id
    WHERE pc.total_plays > 1000 -- Filter out songs with too few plays to have a meaningful ratio
    HAVING ec.playlist_adds > 300
       AND ec.downloads > 200
       -- Add the new repost threshold to ensure social velocity
       AND ec.reposts > 100
    -- Order by the conversion ratio to find the most efficient tracks
    ORDER BY downloads_per_1k_plays DESC
    LIMIT 100;
    """, language="sql")

    st.subheader("3. What to Look For in the Results")
    st.markdown("""
    * **Top Results:** Do the songs at the top of this list *feel* like they have a passionate, core fanbase? Are they different from the results of the first query?
    * **The Ratio:** Is `downloads_per_1k_plays` a useful metric? We should analyze the range of values to see if it effectively separates different kinds of tracks.
    * **Emerging Artists:** Does this query surface more artists who are not yet superstars but have highly engaged audiences?
    """)

# --- U.S. Tab ---
with tab2:
    st.header("United States: Proxy for High-Intent Plays")
    st.markdown("""
    **Hypothesis:** With the `m.source` field being unreliable, we can create a proxy for high-intent listening by analyzing the `source_tab` from the `dw01.events` table. Plays originating from 'search' or 'library' signal active user choice. We will also narrow the play window to better isolate a specific tier of emerging artists.
    """)

    st.subheader("1. The Proposed Test")
    st.markdown("""
    * **Proxy Metric:** A new `high_intent_plays` column will be created by counting plays where `source_tab IN ('search', 'library')`.
    * **Refined Play Window:** The overall play count for a track will be narrowed to `BETWEEN 10,000 AND 50,000` to focus on a more consistent cohort of artists in the "bubbling up" phase.
    * **Thresholds:** We will require a minimum number of these `high_intent_plays` to qualify.
    """)

    st.subheader("2. Polished Query to Test")
    st.code("""
    -- U.S. Test 2: High-Intent Proxy & Refined Window
    -- Focus: Using event source as a proxy for intent, with a narrower play window.
    SELECT
        a.name AS artist_name,
        m.title AS song_name,
        COUNT_IF(e.type = 'play') AS num_plays,
        -- Proxy for high-intent plays: count plays coming from search or library tabs
        COUNT_IF(e.source_tab IN ('search', 'library')) AS high_intent_plays,
        COUNT_IF(e.type = 'download') AS num_downloads
    FROM dw01.events e
    JOIN dw01.music m ON e.object_id = m.music_id_raw
    JOIN dw01.artist a ON m.uploader = a.artist_id
    WHERE e.event_date BETWEEN date_format(current_date - interval '30' day, '%Y%m%d')
                           AND date_format(current_date - interval '1' day, '%Y%m%d')
      AND m.status = 'complete'
      AND e.geo_country = 'US'
    GROUP BY 1, 2
    HAVING
        -- Refined play window to target a specific emerging artist tier
        num_plays BETWEEN 10000 AND 50000
        -- Threshold for downloads remains a strong signal of super-fandom
        AND num_downloads > 250
        -- New threshold for our high-intent proxy
        AND high_intent_plays > 100
    ORDER BY high_intent_plays DESC
    LIMIT 100;
    """, language="sql")

    st.subheader("3. What to Look For in the Results")
    st.markdown("""
    * **Proxy Effectiveness:** How many tracks meet the `high_intent_plays > 100` threshold? If the result is empty, the proxy isn't working or the threshold is too high.
    * **Artist Tier:** Does the `10k-50k` play window produce a more consistent list of artists compared to the previous, wider window?
    * **Qualitative Feel:** Do the songs surfaced by this query sound like tracks that people would actively search for? This requires a listening session.
    """)

# --- Caribbean Tab ---
with tab3:
    st.header("Caribbean: Measuring Engagement Velocity")
    st.markdown("""
    **Hypothesis:** True momentum isn't just about total engagement, but the *rate* at which it's acquired. A track gaining favorites, reposts, and comments *this week* is more relevant than one with high historical totals. We will create a `velocity_score` to measure this acceleration.
    """)

    st.subheader("1. The Proposed Test")
    st.markdown("""
    * **Velocity Metrics:** We will count `favorites`, `reposts`, and `comments` specifically within the **last 7 days**.
    * **Velocity Score:** A weighted score will be calculated: `(favorites_7d * 1.0) + (reposts_7d * 1.5) + (comments_7d * 2.0)`. Reposts and comments are weighted higher as they represent stronger engagement.
    * **New Metric:** We will now include `comments` as a key indicator of community-building.
    """)

    st.subheader("2. Polished Query to Test")
    st.code("""
    -- Caribbean Test 2: Engagement Velocity
    -- Focus: Finding tracks with accelerating engagement (favorites, reposts, comments) in the last 7 days.
    SELECT
        a.name AS artist_name,
        m.title AS song_name,
        -- Total counts over 30 days
        COUNT_IF(e.type = 'favorite') AS total_favorites,
        COUNT_IF(e.type = 'repost') AS total_reposts,
        COUNT_IF(e.type = 'comment') AS total_comments,
        -- Velocity: Counts in the last 7 days
        COUNT_IF(e.type = 'favorite' AND e.event_date >= date_format(current_date - interval '7' day, '%Y%m%d')) AS favorites_last_7d,
        COUNT_IF(e.type = 'repost' AND e.event_date >= date_format(current_date - interval '7' day, '%Y%m%d')) AS reposts_last_7d,
        -- Calculate a velocity score
        (COUNT_IF(e.type = 'favorite' AND e.event_date >= date_format(current_date - interval '7' day, '%Y%m%d')) * 1.0 +
         COUNT_IF(e.type = 'repost' AND e.event_date >= date_format(current_date - interval '7' day, '%Y%m%d')) * 1.5 +
         COUNT_IF(e.type = 'comment' AND e.event_date >= date_format(current_date - interval '7' day, '%Y%m%d')) * 2.0
        ) AS velocity_score
    FROM dw01.events e
    JOIN dw01.music m ON e.object_id = m.music_id_raw
    JOIN dw01.artist a ON m.uploader = a.artist_id
    WHERE e.event_date BETWEEN date_format(current_date - interval '30' day, '%Y%m%d')
      ...
      AND e.type IN ('favorite', 'repost', 'comment')
    GROUP BY 1, 2
    HAVING
        -- Keep original thresholds to ensure a baseline of engagement
        total_favorites > 100
        AND total_reposts > 30
        -- Add a new threshold for comments
        AND total_comments > 10
        -- Ensure there's at least some recent activity
        AND velocity_score > 20
    ...
    st.subheader("3. What to Look For in the Results")
    st.markdown("""
    * **Timeliness:** Does this list feel more "current" than the last one? The top results should be songs that are buzzing on the platform right now.
    * **Score Distribution:** What is the range of the `velocity_score`? Does it clearly identify a top tier of accelerating tracks?
    * **Impact of Comments:** Does adding `comments` to the mix surface different types of artists (e.g., those with a strong lyrical message or controversial topic)?
    """)
