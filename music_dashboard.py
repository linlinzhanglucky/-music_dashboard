...
# Process: Event type analysis
fig_event_type = px.bar(
    event_type_df.sort_values(by="event_count", ascending=False),
    x="event_type",
    y="event_count",
    title="Event Type Count (Last 30 Days)",
    labels={"event_type": "Event Type", "event_count": "Count"},
    hover_data=["unique_users"]
)

# Process: Music engagement metrics
fig_music_engagement = px.bar(
    music_engagement_df.sort_values(by="total_plays", ascending=False).head(20),
    x="artist",
    y="total_plays",
    title="Top Artists by Total Plays (Last 30 Days)",
    labels={"artist": "Artist", "total_plays": "Total Plays"},
    hover_data=["unique_listeners"]
)

# Process: Engagement Ratios
fig_engagement_ratios = make_subplots(specs=[[{"secondary_y": True}]])
fig_engagement_ratios.add_trace(go.Bar(
    x=engagement_ratios_df["artist"],
    y=engagement_ratios_df["favorite_to_play_ratio"],
    name="Favorite to Play Ratio",
    marker_color="#FF8042"
), secondary_y=False)

fig_engagement_ratios.add_trace(go.Scatter(
    x=engagement_ratios_df["artist"],
    y=engagement_ratios_df["unique_users"],
    name="Unique Users",
    mode="markers+lines",
    marker_color="#8884d8"
), secondary_y=True)

fig_engagement_ratios.update_layout(
    title="Artist Engagement Ratios & Listener Base",
    xaxis_title="Artist",
)
fig_engagement_ratios.update_yaxes(title_text="Favorite to Play Ratio", secondary_y=False)
fig_engagement_ratios.update_yaxes(title_text="Unique Users", secondary_y=True)
...
# Display in dataframe for manual inspection in Streamlit
tools.display_dataframe_to_user("Music Engagement & Artist Metrics", music_engagement_df)

(fig_event_type, fig_music_engagement, fig_engagement_ratios, fig_geo)
